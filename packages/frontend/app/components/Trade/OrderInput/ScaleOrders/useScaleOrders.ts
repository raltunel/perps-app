...
export default function useScaleOrders({
    ...
    const [priceDistribution, setPriceDistribution] =
        useState<PriceDistributionType>(PRICE_DISTRIBUTION_TYPES.FLAT);
    const [ratioDistribution, setRatioDistribution] =
        useState<RatioDistributionType>(RATIO_DISTRIBUTION_TYPES.EVENLY_SPLIT);
    ...
    const updateOrderRatio = (index: number, newRatio: number | string) => {
        ...
        const numericRatio =
            typeof newRatio === 'string'
                ? newRatio === ''
                    ? 0
                    : parseFloat(newRatio) || 0
                : newRatio;
        ...
        newOrders[index].quantity = +(
            (newOrders[index].ratio * totalQuantity) /
            100
        ).toFixed(3);
