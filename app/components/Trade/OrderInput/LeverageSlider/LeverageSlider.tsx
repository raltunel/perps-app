...
export default function LeverageSlider({
  ...
  const getKnobPosition = () => {
    return selectedIndex === -1 ? 0 : (selectedIndex / (options.length - 1)) * 100;
  };
        ...
        <div className={styles.sliderContainer}>
          ...
          >
            ...
            {options.map((option, index) => (
              <div 
                key={option.value}
                className={`${styles.sliderMarker} ${index <= selectedIndex ? styles.active : ''} ${index === selectedIndex ? styles.sliderMarkerCurrent : ''}`}
                style={{ 
                  left: `${(index / (options.length - 1)) * 100}%`,
                  backgroundColor: index <= selectedIndex ? 
                    colors[option.value.toString()] || '#FFFFFF' : 
                    'transparent',
                  borderColor: index <= selectedIndex ? 'transparent' : 'rgba(255, 255, 255, 0.3)'
                }}
              ></div>
            ))}
          ...
          <div className={styles.labelContainer}>
            {options.map((option, index) => (
              <div 
                ...
                style={{ 
                  left: `${(index / (options.length - 1)) * 100}%`,
                  color: index <= selectedIndex ? '#FFFFFF' : '#808080'
                }}
