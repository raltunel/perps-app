...
export const useLeaderboard = (initialData: LeaderboardData[]) => {
  ...
  const [filteredData, setFilteredData] = useState<LeaderboardData[]>(initialData);
  ...
  const isPrevButtonDisabled = useMemo(() => currentPage === 1, [currentPage]);
  const isNextButtonDisabled = useMemo(() => currentPage === totalPages, [currentPage, totalPages]);
